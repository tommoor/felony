felony.Enemy = felony.Body.extend({
	...
	initialize: function () {
		...
		this.weapon = new felony.WeaponCannon();
        this.display();
	},

    fire: function () {
		var p = this.weapon.fire(this);
		felony.interface.addChildTo(p, 'enemy_projectiles');
    },
	...
	nextWaypoint: function () {
		...
		this.nextx = (felony.interface.width/100)*this.waypoints[this.current][0];
		this.nexty = (felony.interface.height/100)*this.waypoints[this.current][1];	
	},
	...
	hit: function () {
		felony.particleManager.spark(this, 5);
	...
	kill: function () {
		...
		felony.levelManager.incStatistic('kills');
		felony.levelManager.trigger('kill', this);
		felony.interface.shake = 10;
		this.destroy();
	}
});