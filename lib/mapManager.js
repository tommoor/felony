felony.mapManager = _.extend({
	...
	update: function(vector) {
		...
		var currentY = -Math.round(vector.y/5/felony.game.SCALE) + (this.visibleWidth/2);
		var currentX = -Math.round(vector.x/5/felony.game.SCALE) + (this.visibleWidth/2);
	...
	getMaxWidth: function() {
		return Math.min(felony.map[0].length, this.tileOffset.x+this.visibleWidth-1);
	},
	
	getMaxHeight: function() {
		return Math.min(felony.map.length, this.tileOffset.y+this.visibleHeight-1);
	}
